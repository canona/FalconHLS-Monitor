...
| `checkIntervalSeconds`              | number    | Tần suất kiểm tra ĐẦY ĐỦ Level 1→2→3 mỗi luồng (giây). Ví dụ `30` = kiểm tra mỗi 30 giây. Với nhiều kênh chung 1 origin, giá trị này thường phải khá lớn (xem mục 2.1) — dùng `fastCheckIntervalSeconds` bên dưới để bù lại tốc độ phát hiện sự cố nhẹ. |
| `fastCheckIntervalSeconds`          | number    | *(mới, mặc định `15`)* Tần suất riêng, nhanh hơn nhiều, CHỈ cho Level 1 (manifest) + Level 2 (đóng băng) — hai bước này là HTTP GET nhẹ, không đụng tới `hostQueue`/`ffprobeQueue` bị giới hạn theo origin, nên chạy dày mà không gây quá tải. Phát hiện sớm lỗi manifest/đóng băng trong vài chục giây thay vì phải chờ tới lượt Level 3 chậm. Xem `scheduler.ts::runFastProbe`. |
